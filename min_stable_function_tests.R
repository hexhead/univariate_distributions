# min_stable_function_tests.R - Bill White - 5/19/19
# 
...
s_s <- c(2)
e_s <- c(-0.5, 0.5, 1.5)
raw_results1 <- amstat_min_stable(x_s = x_s, m_s = m_s, s_s = s_s, e_s = e_s) 
raw_results1$Facet = "Case 1: Vary xi"

m_s <- c(-1)
s_s <- c(0.5, 1, 1.2)
e_s <- c(1/2)
raw_results2 <- amstat_min_stable(x_s = x_s, m_s = m_s, s_s = s_s, e_s = e_s) 
raw_results2$Facet = "Case 2: Vary sigma"

m_s <- c(-1.5, 0.5, 1)
s_s <- c(2)
e_s <- c(0.5)
raw_results3 <- amstat_min_stable(x_s = x_s, m_s = m_s, s_s = s_s, e_s = e_s) 
...
plot_obj <- ggplot(raw_results, aes(x = x, 
  ...
  geom_line() + 
  xlab("x") + ylab("PDF") +
  ggtitle(label = "Min Stable Distribution",
