# pearson_type_ix_function_tests.R - Bill White - 3/8/19
# 
...
source("pearson_type_ix_function.R")

x_s <- seq(from = 0, to = 1, by = 0.1)
m_s <- c(2, 3, 4)
raw_results <- amstat_pearson_type_ix(x_s = x_s, m_s = m_s)
head(raw_results)
plot_obj <- ggplot(raw_results, aes(x = x, 
                                    y = y, 
                                    colour = Parameters)) + 
  geom_line() +   xlab("x") + ylab("PDF") + 
  ggtitle(label = "Pearson Type IX", subtitle = "Probability Density Function") +
